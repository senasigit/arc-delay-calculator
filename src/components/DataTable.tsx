  ...
  return (
    <div className="w-full md:w-80 h-full bg-white/5 backdrop-blur-md border-l border-white/10 flex flex-col overflow-y-auto">
      <div className="p-4 border-b border-white/10 sticky top-0 bg-black/20 backdrop-blur-md z-10 print:static print:border-none print:p-0 print:mb-4 print:bg-transparent">
        ...
        <p className="text-xs text-purple-400 print:text-gray-600">Instruksi Fisik dan Kalkulasi Delay (ms)</p>
      </div>
          ...
          {groups.map((group) => (
            ...
            >
              <div className="flex justify-between items-center mb-2">
                <span className="font-semibold text-purple-400 print:text-black">{group.label}</span>
                <div className="flex space-x-1 print:hidden">
                  {cardioidEnabled && (
                    <button 
                      onClick={() => onToggleCardioid(group.positionId)}
                      className={`text-[10px] px-1.5 py-1 rounded font-bold border ${group.cardioidDisabled ? 'bg-gray-800 text-purple-400 border-gray-700 hover:bg-gray-700' : 'bg-zinc-900/80 text-purple-400 border-purple-500/50 hover:bg-purple-600'}`}
                    >
                  ...
                  <button 
                    ...
                    className={`text-[10px] px-1.5 py-1 rounded font-bold border ${group.muted ? 'bg-red-900/50 text-red-400 border-red-800 hover:bg-red-800' : 'bg-gray-800 text-purple-400 border-gray-700 hover:bg-gray-700'}`}
                  >
              ...
              <div className="flex justify-between items-center pb-2 border-b border-gray-800 print:border-gray-300">
                <span className="text-xs text-purple-400 print:text-gray-600">Titik Fisik X (m)</span>
                <span className="text-sm font-medium text-purple-400 print:text-black">{group.x.toFixed(3)} m</span>
              </div>
              ...
              <div className="mt-2 space-y-1.5">
                 ...
                 {[...group.boxes].reverse().map((box) => (
                    <div key={box.stackIndex} className="flex justify-between items-center text-xs">
                       <span className={`w-14 font-medium ${box.isRear ? 'text-purple-400 print:text-purple-500' : 'text-purple-400 print:text-gray-600'}`}>
                          Box {box.stackIndex + 1}
                       </span>
                       
                       <span className={`flex-1 text-center font-bold ${box.isRear || box.polarity === -1 ? 'text-purple-400 print:text-purple-500' : 'text-purple-400 print:text-black'}`}>
                          {box.positionLabel} {box.isRear ? '(Reversed)' : (box.polarity === -1 ? '(Inverted)' : '')}
                       </span>

                       <span className={`w-16 text-right font-bold ${box.isRear ? 'text-purple-400 print:text-purple-500' : 'text-accent print:text-purple-500'}`}>
