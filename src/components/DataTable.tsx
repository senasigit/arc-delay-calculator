      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-left text-sm text-gray-300">
          <thead className="text-xs text-gray-400 uppercase bg-[#0f1115] sticky top-0 z-10 shadow-sm">
            <tr>
              <th className="px-4 py-3 border-b border-dark-border">Box</th>
              <th className="px-4 py-3 border-b border-dark-border">X (m)</th>
              <th className="px-4 py-3 border-b border-dark-border">Delay (ms)</th>
              {cardioidEnabled && (
                <th className="px-4 py-3 border-b border-dark-border">Cardioid</th>
              )}
            </tr>
          </thead>
          <tbody>
            {calculations.map((box) => (
              <tr key={box.index} className="border-b border-dark-border/50 hover:bg-[#1f2229] transition-colors">
                <td className="px-4 py-3 font-medium text-gray-100 whitespace-nowrap" title={box.label}>
                  {box.index + 1}
                </td>
                <td className="px-4 py-3">{box.x > 0 ? '+' : ''}{box.x.toFixed(2)}</td>
                <td className="px-4 py-3 font-bold text-accent">{box.delayMs.toFixed(2)}</td>
                {cardioidEnabled && (
                  <td className="px-4 py-3 font-bold text-green-400">{box.totalCardioidDelayMs.toFixed(2)}</td>
                )}
              </tr>
            ))}
          </tbody>
